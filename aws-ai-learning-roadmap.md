...
| AI-16 | Batch Transform / 离线批量推理 | Batch Transform、S3、Model artifact、CloudWatch | 离线批量推理 dry-run 计划 | 能不用长期 endpoint 理解批量推理 |
| AI-17 | Inference 模式全家桶 | Real-time、Serverless、Async、Batch、MME | 推理模式选型表和架构图 | 能根据延迟、吞吐和成本选择推理方式 |
| AI-18 | Model、Endpoint Config 与 Endpoint | SageMaker Model、Endpoint、IAM、CloudWatch | 实时 endpoint 部署计划或短时测试 | 能解释 endpoint 为什么会持续收费并能正确删除 |
| AI-19 | 大模型参数实验与 HPO | HPO、Training jobs、metrics、LoRA/训练参数 | 一个小规模 HF 参数实验计划 | 能解释 objective metric、search range、best training job 和成本放大 |
...
```text
数据 -> 处理 -> 训练准备 -> 模型产物 -> 批量推理 -> 推理选型 -> 可选实时部署 -> 调参 -> 注册 -> 监控 -> 治理 -> 清理
```
...
Raw data in S3
  -> Processing job
  -> Training job design / model artifact
  -> Batch Transform
...
- 先 batch，后推理模式选型，再 realtime endpoint。
- 先小数据，后 pipeline。
- 默认从本地 VS Code 发起 SageMaker SDK / boto3 调用。
- 先 Hugging Face / PyTorch 框架容器，后自定义容器。
- 先 dry-run 和架构图，后短时 endpoint；serverless / async / autoscaling 放到选型后再看。
...
从 Hugging Face 选择一个小型开源模型
  -> 准备 SageMaker Hugging Face / PyTorch training job
  -> 理解它如何输出 model.tar.gz 到 S3
  -> quota 和费用允许时再真实训练
```
...
- [ ] 理解 CloudWatch Logs 和 training metrics 会在哪里出现。
- [ ] 理解 S3 中的 `model.tar.gz` 会如何被后续部署使用。
...
目标：不用长期 endpoint，也能理解 SageMaker 如何对 S3 文件做批量推理。
...
- [ ] 准备 JSONL 批量输入样例。
- [ ] 写 Batch Transform dry-run request。
- [ ] 解释为什么 Batch Transform 不需要 Endpoint Config / Endpoint。
- [ ] 记录 Batch Transform 和 real-time endpoint 的区别。

验收标准：

- [ ] 能解释 batch input、SageMaker Model、Transform Job、S3 output 的关系。
- [ ] 能说明 Batch Transform 为什么比 endpoint 更适合离线学习场景。
...
- [ ] 画一张推理模式架构图，把 endpoint、async、serverless、batch 的输入输出画清楚。
...
目标：理解实时部署链路，并只在有真实 artifact、quota 和删除计划时做短时测试。
...
- [ ] 用 dry-run 方式写出 CreateModel / CreateEndpointConfig / CreateEndpoint 请求。
- [ ] 解释 endpoint config 和 endpoint 的区别。
- [ ] 记录 endpoint quota、instance type、CloudWatch Logs、删除顺序。
- [ ] 如果有真实 artifact 且准备好删除，再部署一个短时 endpoint。
- [ ] 调用一次 inference 后立刻删除 endpoint、endpoint config、model。
...
- [ ] 先写 HPO request 计划，不急着启动 tuning job。
- [ ] 理解 max jobs、parallel jobs 如何放大成本。
...
AI-10 专用 AI API
  ...
  -> AI-13 Processing Jobs
  -> AI-14 Training Jobs
  -> AI-15 Model artifact / deployment preparation
  -> AI-16 Batch Transform
  -> AI-17 Inference modes
  -> AI-18 Endpoint deployment
  -> AI-19 Hyperparameter Tuning
  -> AI-20 Model Registry
  -> AI-21 Pipelines
